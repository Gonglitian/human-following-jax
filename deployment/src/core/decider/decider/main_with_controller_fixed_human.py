#!/usr/bin/env python3
"""
Fixed Human + P Controller 版本
使用固定坐标作为目标，P Controller 根据距离自动选择 preference
"""
...
class PController:
    """P Controller 根据距离自动选择 preference"""
    
    def __init__(self, kp=4.0, target_distance=2.0, tolerance=0.10):
        self.kp = kp
...
class Decider(Node):
    def __init__(self):
        ...
        log_filename = datetime.now().strftime("decider_fixed_controller_%Y%m%d-%H%M%S.log")
        ...
        # P Controller
        self.p_controller = PController(kp=4.0, target_distance=2.0, tolerance=0.10)
        self.target_following_distance = 2.0
        ...
        self.use_fixed_target = True
        self.fixed_target_position = (2.4, 0)
    ...
    def build_observation_from_humans(self):
        ...
        if self.use_fixed_target:
            ...
            # 固定目标放第一位，其他检测到的人也加入（用于避障）
            sorted_humans = [(-1, fx, fy, dist)] + self.tracked_humans[:nHumansMax-1]
        else:
    ...
    def command_callback(self, msg: String):
        ...
        if self.current_mode_ in ("automatic","combined"):
            if cmd_str == "auto:forward":
                ...
                self.get_logger().info("[Decider] Human following mode activated (Fixed Human + Controller).")
                self.forward_mode = True
                self.reset_stats()
