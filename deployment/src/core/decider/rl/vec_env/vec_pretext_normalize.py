...
class VecPretextNormalize(VecEnvWrapper):
    ...
    def __init__(self, venv, ob=False, ret=False, clipob=10., cliprew=10.,
        ...
        load_path = os.path.join(os.getcwd(), self.config.pred.model_dir) # yjp mark: the model in gst_updated is for prediction
        ...
        # self.pre_interval is the frequency of predictions, meaning that the pred model is called every self.pred_interval steps
        
        self.buffer_len = (self.args.obs_seq_len - 1) * self.pred_interval + 1
        # self.buffer_len is for the number of observations to be fed into the pred model
        # self.pred_interval is for the frequency of predictions
        # the calculation of self.buffer_len is based on the assumption that the pred model is called every self.pred_interval steps
    ...
    # def cost2Env_async(self, cost): # yjp mark: pass in predictions to envs
    #     self.venv.cost2Env_async(cost)

    # def cost2Env_wait(self):
    #     outs=self.venv.cost2Env_wait()
    #     return outs
    ...
    def process_obs_rew(self, O, done, rews=0., infos=()): # yjp mark: O for observation
        # yjp mark: this function is where prediction models are called
        ...
        # yjp mark: dims of out_traj: [num_envs, agent_indices, timesteps, features]
        new_spatial_edges = out_traj[:, :, :, :2].reshape(self.num_envs, self.max_human_num, -1)
        
        # yjp mark: the first two features of O['spatial_edges'] are positions at timestep 0, 
        # which is not contained in out_traj
        O['spatial_edges'][:, :, 2:][out_mask] = new_spatial_edges[out_mask] # yjp mark: from placeholders to meaningful values

        # O['spatial_edges'][:, :, 2:][~out_mask] = human_pos.unsqueeze(2).repeat(1, 1, self.config.sim.predict_steps * 2)[1 - out_mask] 

        # sort all humans by distance to robot
        # [nenv, human_num]
        # 输出给 gst 的是 unsorted, 但是输出给 RL 的是 sorted
